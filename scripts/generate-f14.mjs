...
const BLUE='#6cb6ff', TEAL='#4ec9b0', VIOLET='#c6a0f6', SKY='#87CEFA', CORAL='#ff9f7a', GREEN='#8bd450', LINE='#31516a';
...
const seriesColors=[BLUE,TEAL,VIOLET,SKY];
const parts=[];
...
const first=rows[0],last=rows.at(-1); parts.push(`<circle cx="${px(first.x[0])}" cy="${py(first.x[1])}" r="11" fill="${GREEN}"/><circle cx="${px(last.x[0])}" cy="${py(last.x[1])}" r="11" fill="${SKY}"/>`);
